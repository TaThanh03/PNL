...
/**
  ...
  *                    mineure
  *
  * @from: commit qui devcommiendra le predecesseur du commit insere
...
struct commit *add_minor_commit(struct commit *from, char *comment)
{
	...
	printf("minor commit added: %2u.%ld  %s\n", comm->version.major, comm->version.minor, comm->comment);
...
struct commit *add_major_commit(struct commit *from, char *comment)
{
	...
	printf("major commit added: %2u.%ld  %s\n", comm->version.major, comm->version.minor, comm->comment);
	return insert_commit(from, comm);
}
